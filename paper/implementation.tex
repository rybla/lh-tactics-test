...
\LC{induct}) that facilitate SMT reasoning. We show how a \LangATerm
...
Based on the declaration of \LC{concatElem} inside the quasiquotter,
...
%
The proof macro sequence is then preprocessed to append \macro{auto} \LC{[] 3}
at its end, as it doesn't end with an explicit evidence macro.

\todo{(elem x ys) should be {elem x ys}, but causes {too many {}} error} %
%
The first proof macro in the sequence is \LC{assert (elem x ys)},
...
position: \LC{xs'}.
Observe that this list of neutral forms appears in the \LC{Auto} structure in the 
...
Of course, it turns out that not all of these terms are needed for a valid
proof. The pruning process, uses the \LC{Auto} structure's metadata statefully
to safely prune such unnecessary terms.
...
\section{Explicitly Introduced Variables}
\hen{
