...
Note that the only proof macros that can introduced variables into context are
\macro{induct} and \macro{destruct}. These control flow macros do not by default
...
example, if the user wanted to \macro{induct} on a list and then later
\macro{destruct} the tail of that list. So, in a similar style to Coq's
\LC{induction} and \LC{destruct} tactics, the proof macro system provides the
following optional syntax for explicitly introducing variables by name with
\macro{induct} and \macro{destruct}.
\begin{align*}
...
scope. For example, if the user \macro{induct}s on a list and then later
\macro{destruct}s on the tail of the list, the later \macro{destruct}ion should
...
\end{align*}
So, the user that wants to \macro{induct} on a list \LC{xs :: [N]} and then
\macro{destruct} on the tail of the list can write the following.
\begin{code}
  induct xs as [ / x' xs'];
