...
This expansion process is formalized in
Figure~\ref{fig:proof-macro-semantics}.



Sequences of macros are then
expanded into a \LangBTerm, taking into account the partial expansion
of any already expanded prefix; we formalize this expansion in
Figure~\ref{fig:proof-macro-semantics}.


We can broadly identify two types of proof macros: {\em control-flow}
and {\em evidence} macros. Control flow macros correspond to proof
terms that alter the control flow of the program, such as pattern
matching.  Evidence macros are processed into terms that provide
evidence to the Liquid Haskell typechecker, such as introducing a
lemma to the refinement context.
...
\\  
  \inference{\Gamma \vdash a_i:A_i && n \in \mathbb{N}}
  {\Gamma; \Rho
  \vdash 
